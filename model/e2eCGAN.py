...
class e2eCGAN(template.GAN):
    ...
    def get_name(self):
        return 'e2eCGAN_' + str(self.basic_kernel_size()) + '_' + str(self.basic_dilation_rate())
    ...
    def gen_generator_loss(self):
        ...
        return tf.keras.layers.Lambda(
            lambda x:(
                ...
                0.35 * encoder_loss(x['color'], x['encoder_output']) +
                1.25 * decoder_loss(x['gray'], x['decoder_output']) +
                7 * tf.expand_dims(x['discriminator_output'], axis=1)/1000.
            ),
            trainable=False,
            name='generator_loss'
        )
    ...
    def gen_discriminator_model(self):
        ...
        cover_image_in = layers.Input(shape=(self.image_H, self.image_W, 3), name="cover_image_0")
        ...
        cover_image = cover_image_in

        layer_in = layers.Concatenate(axis=3, name='in_extended_layer_1')([connected_images, cover_image])
        ...
        return tf.keras.Model([connected_images_in, cover_image_in], [layer_out], name="decoder_model")