...
class e2eCNN(template.EncoderDecoder):
    ...
    def gen_encoder_model(self):
        ...
        N = 4
        for i in range(N):
            conn_layer_in = layers.Concatenate(axis=3,
                name='encoder_in_' + str(i))([gray_layer, conn_layer])

            conn_layer = layers.Conv2D(filters=16, kernel_size=3, padding='same', activation='relu',
                                   name='encoder_color_A_' + str(i))(conn_layer_in)
            gray_layer = layers.Conv2D(filters=16, kernel_size=3, padding='same', activation='relu',
                name='encoder_gray_A_' + str(i))(gray_layer)

            conn_layer = layers.Conv2D(filters=16, kernel_size=(3 if i+1 < N else 1), activation='relu',
    ...
    def gen_decoder_model(self):
        ...
        layer_1 = layers.Conv2D(filters=16, kernel_size=3, activation='relu',
            padding='same', name='decoder_l_1')(layer_0)

        layer_2 = layers.Conv2D(filters=16, kernel_size=3, activation='relu',
            padding='same', name='decoder_l_2')(layer_1)

        layer_3 = layers.Conv2D(filters=8, kernel_size=3, activation='relu',
            padding='same', name='decoder_l_3')(layer_2)

        layer_4 = layers.Conv2D(filters=8, kernel_size=3, activation='relu',
            padding='same', name='decoder_l_4')(layer_3)

        layer_5 = layers.Conv2D(filters=3, kernel_size=3, activation='relu',
